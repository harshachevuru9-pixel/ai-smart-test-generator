import { Question, QuestionType, DifficultyLevel } from '../src/types/index.js';
...
export async function generateAIQuestions(options: GenerateOptions): Promise<Question[]> {
  ...
  if (apiKey) {
    try {
      ...
      console.warn('Gemini API call failed, falling back to Smart AI Generator engine:', err);
    }
  }

  // Fallback to internal Smart AI Curriculum Generator engine
  return generateSmartFallbackQuestions(options);
}

async function generateWithGeminiAPI(options: GenerateOptions, apiKey: string): Promise<Question[]> {
  const prompt = `
...
[
  {
    ...
    "marks": 2,
    "timeLimit": 45 // in seconds (30 to 120 based on complexity)
  }
]
`;
...
function generateSmartFallbackQuestions(options: GenerateOptions): Question[] {
  const { subject, topic, description, instructions, count, difficulty, questionTypes } = options;
  const questions: Question[] = [];

  const effectiveTypes = questionTypes.length > 0 ? questionTypes : ['mcq', 'true_false', 'short_answer', 'fill_in_blanks'];
  
  const sampleBank: Record<string, any[]> = {
    'data structures': [
      {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const itemDiff: 'easy' | 'medium' | 'hard' = difficulty === 'mixed' 
      ? (i % 3 === 0 ? 'easy' : i % 3 === 1 ? 'medium' : 'hard') 
      : difficulty as any;

    let questionText = template.question;
    if (topicKey === 'default') {
      questionText = `Q${i + 1} (${topic}): ` + template.question;
    } else {
      questionText = `Q${i + 1}: ` + template.question;
    }
    ...
    if (qType === 'mcq' && template.type !== 'mcq') {
      ...
      questions.push({
        ...
        correctAnswer: topic.split(' ')[0] || 'framework',
      ...
      questions.push({
        ...
        questionType: template.type as any,
