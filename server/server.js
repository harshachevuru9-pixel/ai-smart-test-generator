...
const DATA_FILE = process.env.DATA_FILE || path.join(__dirname, 'data_store.json');
...
app.post('/api/auth/signup', (req, res) => {
  ...
  const newAdmin = { id: `admin_${Date.now()}`, name, email, role: 'admin' };
...
app.get('/api/tests/:id/analytics', (req, res) => {
  ...
  res.json({
    ...
    analytics: {
      ...
      activeCount,
      completedCount,
      joinedCount: attempts.filter(a => a.status === 'joined').length,
...
app.get('/api/network-info', (req, res) => {
  ...
  const hostHeader = req.headers.host || `${ip}:${PORT}`;
  const protocol = req.protocol || 'http';
  res.json({
    success: true,
    localIp: ip,
    frontendUrl: `${protocol}://${hostHeader}`,
    backendUrl: `${protocol}://${hostHeader}`
  });
});
...
app.get('*', (req, res) => {
  ...
  if (fs.existsSync(distIndex)) {
    ...
    res.send('AI Smart Test Generator Server Running. Build frontend using npm run build.');
  }
});
...
httpServer.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`AI Smart Test Generator Server running on:`);
